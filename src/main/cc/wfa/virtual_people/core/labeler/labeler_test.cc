...
TEST(LabelerTest, TestBuildFromRoot) {
  ...
  ASSERT_OK_AND_ASSIGN(
      std::unique_ptr<Labeler> labeler, Labeler::Build(root));
  ...
  EXPECT_THAT(vpid_counts, UnorderedElementsAre(
...
TEST(LabelerTest, TestBuildFromNodesMultipleNodesWithoutIndex) {
  std::vector<CompiledNode> nodes;
  nodes.emplace_back();
  ASSERT_TRUE(google::protobuf::TextFormat::ParseFromString(R"pb(
      name: "TestNode1"
