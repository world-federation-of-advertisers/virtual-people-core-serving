...
absl::Status UpdateMatrixImpl::Update(LabelerEvent& event) const {
  ...
  if (hash_matcher_) {
    column_index = hash_matcher_->GetMatch(event);
  } else if (filters_matcher_) {
    column_index = filters_matcher_->GetFirstMatch(event);
  } else {
  ...
  int row_index = row_hashings_[column_index]->Hash(
